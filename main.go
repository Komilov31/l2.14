...
// если 1 канал передан, то возвращаем его если больше то переходим в default
// считаем из 2 каналов, приходит значение -> селект закончится и канал закрываем
// если то рекурсивно запускаем эту функцию, что повторяет
func or(channels ...<-chan interface{}) <-chan interface{} {
	mergedChan := make(chan interface{})
